...
import { Table } from '@/types/table'; // Assuming you have this type defined
import BulkTableActions from './BulkTableActions'; // Import the new component
...
export default function AdminTablesContent() {
  ...
  // NEW STATE: State for selected table IDs (using a Set for efficient lookups)
  ...
  // NEW FUNCTION: Toggles selection for a single table
  ...
  // NEW FUNCTION: Toggles selection for all tables
  const toggleSelectAll = () => {
    if (selectedTableIds.size === tables.length && tables.length > 0) {
      // All are currently selected (and there are tables), so deselect all
      setSelectedTableIds(new Set());
    } else {
      // Not all are selected (or none), so select all available tables
      const allTableIds = new Set(tables.map(table => table._id));
      setSelectedTableIds(allTableIds);
    }
  };

  // NEW FUNCTION: Clears all selected tables
  ...
  const fetchTables = async () => {
    ...
    try {
      ...
      setTables(data.data);
      clearSelection(); // Clear selection when tables are re-fetched
    } catch (err: any) {
      console.error('Error fetching tables:', err);
      showMessage(`Failed to fetch tables: ${err.message}`, true); // Fixed template literal
    } finally {
  ...
  const handleAddTable = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showMessage('Table added successfully!'); // Fixed typo
      clearSelection(); // Clear selection after adding
    } catch (err: any) {
      console.error('Error adding table:', err);
      showMessage(`Failed to add table: ${err.message}`, true); // Fixed template literal
    }
  };
  ...
  const handleEditClick = (table: Table) => {
    ...
    clearSelection(); // Clear selection when editing a single table
  };
  ...
  const handleUpdateTable = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error updating table:', err); // Fixed typo
      showMessage(`Failed to update table: ${err.message}`, true); // Fixed template literal
    }
  };

  //--- Handlers for Deleting Tables ----
  const handleDeleteTable = async (tableId: string) => {
    // IMPORTANT: Do NOT use window.confirm or alert in the canvas environment.
    // Instead, use a custom modal UI for confirmation.
    // For now, I'll keep the confirm but you should replace it with a modal.
    ...
    try {
      ...
      showMessage(`Failed to delete table: ${err.message}`, true); // Fixed template literal
    }
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {successMessage && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-full max-w-lg bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded shadow-lg text-center" role="alert">
          <strong className='font-bold'>Success!</strong>
          <span className='block sm:inline ml-2'>{successMessage}</span>
        </div>
      )}
      {error && (
        <div className='fixed top-4 left-1/2 -translate-x-1/2 z-50 w-full max-w-lg bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded shadow-lg text-center' role="alert">
      ...
      {/* NEW: Bulk Table Actions Component */}
      <BulkTableActions
        selectedTableIds={selectedTableIds}
        tables={tables}
        onBulkUpdateComplete={fetchTables} // Re-fetch all tables after bulk update
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  <input
                    type="checkbox"
                    // Check if all tables are selected; also handle case with no tables
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {tables.map((table) => (
                <tr key={table._id}>
                  {/* NEW: Checkbox for individual table */}
                  <td className="px-6 py-4 whitespace-nowrap">
                    <input
                      type="checkbox"
                      checked={selectedTableIds.has(table._id)}
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500">
                    ...
                    <div className="mt-1 text-xs text-blue-600 flex items-center">
                      <a href={getCustomerScanURL(table.qrCodeIdentifier)} target="_blank" rel="noopener noreferrer" className="hover:underline">
                        Customer Scan Link
                      </a>
                      <button
                        onClick={() => {
                          ...
                          const textToCopy = getCustomerScanURL(table.qrCodeIdentifier);
